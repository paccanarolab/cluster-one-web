...
function AppContextProvider ({ children }) {
    ...
    const uploadFilePpi = async (inputElement) => {
        ...
        if (file) {
            ...
            try {
                const response = await fetch("http://localhost:8000/v1/api/graph/ppi/", {
                    method: "POST",
                    body: formData,
                });
    ...
    const uploadGoaFile = async (inputElement) => {
        ...
        if (file) {
            ...
            formData.append("file", file);
            try {
                const response = await fetch("http://localhost:8000/v1/api/enrichment/upload/goa/", {
                    method: "POST",
                    body: formData,
                });
                const data = await response.json();
    ...
    const updateRedis = async (ppi_id) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/v1/api/graph/ppi/preloaded/update/?file_name=${ppi[0].file_name}`, {
                method: 'POST'
            });
            const data = await response.json();
            return data;
    ...
    const quickRunClusterOne = async (ppi_id) => {
        ...
        try {
            if ( goaFileName === "" ) {
                var baseUrl = `http://localhost:8000/v1/api/cluster_one/run/?pp_id=${ppi_id}`;
            } else {
                var baseUrl = `http://localhost:8000/v1/api/cluster_one/run/?pp_id=${ppi_id}&goa_file=${goaFileName}`
            }
    ...
    const getAllPpi = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/v1/api/graph/ppi/all/`, {
                method: 'GET'
            });
    ...
    const getProteinDataByCluster = async (clusterId) => {
        // Get all information about all PPIs in the database
        try {
            const response = await fetch(`http://localhost:8000/v1/api/protein/interactions/${clusterId}/?cluster_id=${clusterId}`, {
                method: 'GET'
            });
            const data = await response.json();
    ...
    const getProteinInfo = async (proteinLabel) => {
        // Get all information about all PPIs in the database
        try {
            const response = await fetch(`http://localhost:8000/v1/api/protein/${proteinLabel}/uniprot/?protein=${proteinLabel}`, {
                method: 'GET'
            });
            const data = await response.json();
            setProteinInfo(data);
    ...
    const getEnrichmentData = async (clusterId) => {
        // Get all information about all PPIs in the database
        try {
            const response = await fetch(`http://localhost:8000/v1/api/enrichment/complex/${clusterId}/?cluster_id=${clusterId}`, {
                method: 'GET'
            });
            let data = await response.json();
    ...
    const getOraScore = async (executionId) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/v1/api/enrichment/params/?param_id=${executionId}`, {
                method: 'GET'
            });
            const data = await response.json();
            setOraScore(data);
